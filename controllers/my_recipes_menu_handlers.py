...
class WaitingUserRecipeStateHandler(BaseStateHandler):
    ...
    def get_keyboard(self, session_data: dict):
        error = session_data["context"].get("error")
        recipe_to_add = session_data["context"].get("recipe_to_add")
        recipe_photo = session_data["context"].get("recipe_photo")

        # Показываем кнопку "Отправить заново", если есть ошибка и есть что отправлять
        if error and (recipe_to_add or recipe_photo):
            return error_keyboard("send_recipe_to_llm")

        return step_back_or_cancel_keyboard("back", "to_main")

    async def _process_llm_text(self, event, session_data: dict, text: str):
    ...
    async def _process_llm_photo(self, event, session_data: dict, image_base64: str):
        """Отправляет фото в LLM и обрабатывает ответ."""
    ...
    async def handle_message(self, message: Message, session_data: dict):
        ...
        cmd = await self._process_llm_text(message, session_data, text)
        if cmd:
    ...
    async def handle_photo(self, message: Message, session_data: dict):
        ...
        cmd = await self._process_llm_photo(message, session_data, image_base64)
        if cmd:
            return cmd, session_data
        return None, session_data
    ...
    async def handle_event(self, event: MessageEvent, session_data: dict):
        ...
        if cmd == "send_recipe_to_llm":
            recipe_text = session_data["context"].get("recipe_to_add")
            recipe_photo = session_data["context"].get("recipe_photo")

            if recipe_photo:
                cmd = await self._process_llm_photo(event, session_data, recipe_photo)
            elif recipe_text:
                cmd = await self._process_llm_text(event, session_data, recipe_text)
            else:
                cmd = None
