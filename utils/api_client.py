...
class BreadlabAPIClient:
    ...
    @classmethod
    def get_or_create_session(cls) -> aiohttp.ClientSession:
    ...
    async def close(cls) -> None:
        """Закрывает HTTP-сессию (вызывается при остановке бота)."""
    ...
    @classmethod
    async def post(cls, endpoint: str, request_data: dict) -> Tuple[Optional[dict], Optional[str]]:
    ...
    @classmethod
    async def get(cls, endpoint: str, params: dict = None) -> Tuple[Optional[dict], Optional[str]]:
    ...
    @classmethod
    async def patch(cls, endpoint: str, request_data: dict) -> Tuple[Optional[dict], Optional[str]]:
    ...
    @classmethod
    async def get_user_recipes(cls, external_id: str, page: int = 1) -> Tuple[Optional[dict], Optional[str]]:
        """Получает список рецептов пользователя с пагинацией."""
    ...
    @classmethod
    async def get_recipe(cls, recipe_id: str) -> Tuple[Optional[dict], Optional[str]]:
        """Получает один рецепт по его ID."""
        return await cls.get(f"/recipes/{recipe_id}/")

    @classmethod
    async def delete_recipe(cls, recipe_id: str) -> Tuple[Optional[dict], Optional[str]]:
    ...
    @classmethod
    async def save_recipe(cls, user_id: str, recipe_data: dict, parent_id: int = None) -> Tuple[Optional[dict], Optional[str]]:
        """Сохраняет рецепт (создаёт новый или как версию)."""
        request_data = {
            "user_id": user_id,
            "parent_id": parent_id,
    ...
    @classmethod
    async def patch_recipe(cls, recipe_id: str, recipe_data: dict) -> Tuple[Optional[dict], Optional[str]]:
    ...
    async def check_recipe_exists(cls, user_id: str, title: str) -> Tuple[bool, Optional[str]]:
        """Проверяет, существует ли у пользователя рецепт с таким названием."""
    ...
    async def multiply_recipe(cls, multiplier: int, recipe: dict) -> Tuple[Optional[dict], Optional[str]]:
        """Умножает рецепт на заданный множитель."""
    ...
    @classmethod
    async def recognize_recipe_text(cls, recipe: str) -> Tuple[Optional[dict], Optional[str]]:
        """Отправляет текст рецепта в LLM для распознавания."""
        request = {"recipe": recipe}
        return await cls.post("/recognize_text/", request)
